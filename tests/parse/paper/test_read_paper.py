...
from labridge.parse.paper.paper_keyword_index import get_paper_nodes_keyword_index
from labridge.parse.paper.extractors.source_analyze import PaperSourceAnalyzer
...
def test_read_single_paper():
	...
	for doc_node in doc_nodes_1:
		print("\t>>> ", doc_node.source_node.node_id)
...
def test_read_papers():
	...
	for idx, doc_node in enumerate(nodes_list):
		...
		key_index = get_paper_nodes_keyword_index(paper_nodes=[doc_node,], max_keywords_num=4, llm=llm,
												  storage_context=None)
		print(key_index.index_struct.table)
...
def test_read_directory(paper_dir = None):
	...
	for idx, doc_nodes in enumerate(nodes_list):
		print(f"=========================================== Paper {idx} ===========================================")
		node = doc_nodes[0]
		print(">>> Source: \n", node.source_node.node_id)
		print(">>> Metadata: \n", node.metadata)
...
def test_paper_to_index(input_dirs, persist_dir):
	...
	for idx, input_dir in enumerate(input_dirs):
		...
		key_index = get_paper_nodes_keyword_index(paper_nodes=nodes, max_keywords_num=4, llm=llm,
												  storage_context=my_storage_context)
		key_index.set_index_id(index_id=f"key_index_{idx + 1}")
		key_index.storage_context.persist(persist_dir=persist_dir)
