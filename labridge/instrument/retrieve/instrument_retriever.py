...
def format_instrument_node_batch_fn(
	instrument_nodes: List[BaseNode]
) -> str:
	r"""
...
class InstrumentRetriever:
	...
	def _retrieve_proper_instrument(self, retrieve_items: str):
		r""" Use LLM to select the proper instruments based on their description. """
	...
	async def _aretrieve_proper_instrument(self, retrieve_items: str):
		r""" Asynchronously select the proper instruments based on their description. """
		instrument_ids = self.instrument_store.get_all_instruments()
		dsc_nodes = self.instrument_store.get_nodes(node_ids=instrument_ids)
	...
	def retrieve(
		...
		Returns:
			The retrieved nodes. The contents of these retrieved nodes will be presented as the output.
		"""
