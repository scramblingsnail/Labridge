...
class ShortMemoryManager(object):
	...
	def pack_time_key(self, date_str: str, time_str: str) -> str:
		return f"{date_str} {time_str}"

	def unpack_time_key(self, time_key: str) -> datetime.datetime:
	...
	def load_memory(self, user_id: str) -> Optional[List[ChatMessage]]:
		...
		last_datetime = self.unpack_time_key(time_key=time_key)
	...
	def save_memory(self, user_id: str, chat_history: List[ChatMessage]):
		date, h_m_s = get_time()
		time_key = self.pack_time_key(date_str=date, time_str=h_m_s)
		store_dict = {
